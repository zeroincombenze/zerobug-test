...
class MainTest(test_common.TransactionCase):
    ...
    # --  Primitive Odoo API  --
    # --------------------------
    def browse_rec(self, resource, ids):
        ...
        if self.odoo_major_version <= 7:
            if isinstance(id, (list, tuple)):
                res = []
                for id in ids:
                    res.append(self.registry(resource).browse(self.cr, self.uid, id))
                return res
        elif self.odoo_major_version == 8:
            return self.env[resource].browse(self.cr, self.uid, id)
        else:
            return self.env[resource].browse(id)
        ...
        if len(res):
            if (
                ...
                if fmt == "cmd":
                    ...
                    for id in ids:
                        res |= self.browse_rec(resource, id)
        else:
            res = False
            if fmt:
        ...
        * I xref is invalid, xref is used to search record
        ...
        if isinstance(xref, (int, long)):
            if not resource:  # pragma: no cover
                ...
                return False
            record = self.browse_rec(resource, xref)
        ...
        if len(record) == 1:
            return self.browse_rec(resource, record[0].id)
        if raise_if_not_found:
