...
class MainTest(SingleTransactionCase):
    ...
    def move_translated_xref(self, xref, tnxl_xref, group=None):
        group = group or "base"
        ...
        if tnxl_xref != xref:
            if (
                ...
                del self.setup_data[group][xref]
    ...
    def add_translation_xref(self, xref, tnxl_xref, resource=None, group=None):
        ...
        if ir_resource:
            self.move_translated_xref(xref, tnxl_xref, group=group)
    ...
    def cast_types(self, resource, values, fmt=None, group=None):
        ...
        for field in [x for x in list(values.keys())]:
            ...
            if field == "company_id":
                ...
                bin_contents = self.get_binary_contents(values[field])
                if bin_contents:
                    values[field] = bin_contents
                    continue
                self._logger.info(
                    "🐞cast_type(%s, %s): no binary file found!" % (
                        resource, values[field])
                )
            values[field] = self.tnxl_field_value(
                resource, field, values[field], fmt=fmt)
    ...
    def setup_company(self, company, xref=None, partner_xref=None, values={}):
        add_alias = True
        if not company:
